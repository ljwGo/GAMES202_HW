...
async function GAMES202Main() {
	...
	let box2Transform = setTransform(0, -35, 0, 20, 20, 20);
	...
	for (let i = 0; i < envmap.length; i++) {
		...
		let lineArray = [[], [], []];
		precomputeLT[i] = []
		for (let j = 0; j < Number(preArray.length); j++) {
		...
		// 修改了光的球谐系数的存储格式,改为分别存储rgb的9个sh系数
		precomputeL[i] = val.split(/[(\r\n)\r\n]+/);
		precomputeL[i].pop();
		for (let j = 0; j < 9; j++) {
			rgb = precomputeL[i][j].split(' ');
			for (let k = 0; k < 3; k++) {
				lineArray[k][j] = Number(rgb[k]);
			}
		}
		precomputeL[i] = lineArray;
	}
