...
namespace ProjEnv
{
                      ...
                      int &channel)
    {
        ...
        for (int i = 0; i < 6; i++)
        {
            ...
            if (i == 0)
            {
                // save first image width and height
                width = w;
                height = h;
                channel = c;
            }
            ...
            {
                // 6 images have all the same resolution
    ...
    {
        // atan2 use y / x. value scope is [-pi, pi].
                   ...
                   const int &height)
    {
        ...
        // u_ and v_ are the texel index[0..res-1] on the current face.
        ...
        // x0, y0, x1, y1 in four edge
        float x0 = u - invResolutionW;
        float y0 = v - invResolutionH;
        float x1 = u + invResolutionW;
        float y1 = v + invResolutionH;
        // use [1, 1]~[0, 0] - [1, 0.8]~[0, 0] - [0.8, 1]~[0, 0] + [0.8, 0.8]~[0, 0]
            ...
            for (int y = 0; y < height; y++)
            {
                for (int x = 0; x < width; x++)
                {
                    ...
                    Eigen::Array3f Le(images[i][index + 0], images[i][index + 1],
                                      images[i][index + 2]);
                    // 使用黎曼积分求光的投影系数(假设是单位球)
                    float deltaSolidAngle = CalcArea(x, y, width, height);
                    float shVal = 0;
                    int ix = 0;

                    for (int l = 0; l <= SHOrder; ++l) {
                        for (int m = -l; m <= l; ++m) {
                            // bug01: EvalSH第三个参数类型是Vector3d，传入的dir是Vector3f，类型不匹配
                            // 转换类型为double，不是Vector3d
                            shVal = sh::EvalSH(l, m, dir.cast<double>());
                            ix = sh::GetIndex(l, m);
                            SHCoeffiecents[ix][0] = SHCoeffiecents[ix].x() + Le.x() * shVal * deltaSolidAngle;
                            SHCoeffiecents[ix][1] = SHCoeffiecents[ix].y() + Le.y() * shVal * deltaSolidAngle;
                            SHCoeffiecents[ix][2] = SHCoeffiecents[ix].z() + Le.z() * shVal * deltaSolidAngle;
                        }
                    }
                }
            }
...
public:
    ...
    virtual void preprocess(const Scene *scene) override
    {
        ...
        // fout << "vertices count: "<<mesh->getVertexCount() << std::endl;
        for (int i = 0; i < mesh->getVertexCount(); i++)
        {
            ...
            auto shFunc = [&](double phi, double theta) -> double {
                ...
                if (m_Type == Type::Unshadowed)
                {
                    ...
                    // 计算light transport的函数值
                    return fmax(0, wi.dot(n.normalized()));
                }
                else
                {
        ...
        if (m_Type == Type::Interreflection)
        {
            ...
            MatrixXf points = mesh->getVertexPositions();
            MatrixXf normals = mesh->getVertexNormals();
            
            for (int i=0; i<mesh->getVertexCount(); i++){
                const Point3f& point = points.col(i);
                const Point3f& normal = normals.col(i);
                
                auto coefs = IndirectLightTransportSH(scene, m_TransportSHCoeffs, point, normal, m_Bounce);
            
                for (int j=0; j<SHCoeffLength; j++){
                    m_TransportSHCoeffs.col(i).coeffRef(j) += (*coefs)[j];
                }
            }
        }
        ...
        std::cout << "Computed light transport SH coeffs"
    ...
    std::unique_ptr<std::vector<double>> IndirectLightTransportSH (const Scene *scene, Eigen::MatrixXf vertexLightTransportSH, 
        const Point3f& point, const Normal3f& normal, int bound){
        // 结果
        std::unique_ptr<std::vector<double>> result = std::unique_ptr<std::vector<double>>(new std::vector<double>());
        (*result).assign(SHCoeffLength, 0.0);
        ...
        bound--;
        int sampleSide = static_cast<int>(floor(sqrt(m_SampleCount)));
        ...
        for (int i=0; i < sampleSide; i++){
            for (int j=0; j < sampleSide; j++){
                double alpha = (i + rng(gen)) / sampleSide;
                double beta = (j + rng(gen)) / sampleSide;
                ...
                Ray3f ray(point, wi);
                Intersection its;
                if (wi.dot(normal) > 0 && scene->rayIntersect(ray, its)){
                    // 利用中心坐标插值计算间接光对该点的光传输积分
                    Point3f triangleIxs = its.tri_index;
                    Point3f bary = its.bary;
                    Point3f hitPoint = its.p;

                    // 递归下一次碰撞的结果
                    MatrixXf itsNormals = its.mesh->getVertexNormals();

                    Normal3f itsNormal = Normal3f
                        (itsNormals.col(triangleIxs.x()).normalized() * bary.x() +
                        itsNormals.col(triangleIxs.y()).normalized() * bary.y() +
                        itsNormals.col(triangleIxs.z()).normalized() * bary.z())
                        .normalized();

                    // 这个递归可能造成指数爆炸
                    std::unique_ptr<std::vector<double>> next = IndirectLightTransportSH(scene, vertexLightTransportSH, hitPoint, itsNormal, bound);
                    ...
                    for (int i=0; i < 3; i++){
                        for (int j=0; j < SHCoeffLength; j++){
                            float coef = vertexLightTransportSH.col(triangleIxs[i]).coeffRef(j);
                            (*result)[j] += coef * bary[i];
                        }
                    }
                }
            }
        ...
        // 除以采样数作为权重
        for (int i=0; i < SHCoeffLength; i++){
            (*result)[i] /= sampleSide * sampleSide;
        }
