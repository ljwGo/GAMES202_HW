...
function GAMES202Main() {
	...
	// Init gl
	// Edit start. Webgl2是浏览器内置的
	gl = canvas.getContext('webgl2');
	if (!gl) {
		alert('Unable to initialize WebGL. Your browser or machine may not support it.');
		return;
	}
	// gl.getExtension('OES_texture_float');  // 浮点纹理在webgl2中成为标准了
	// gl_draw_buffers = gl.getExtension('WEBGL_draw_buffers');  // 多纹理绘制同理
	...
	camera.minValueMinmapFBOs = [];
	let lastTextureWidth = canvas.width;
	let lastTextureHeight = canvas.height;
	let textureWidth;
	let textureHeight;

	while (lastTextureWidth > 2 && lastTextureHeight > 2){
		// 使用floor和ceil都会导致误差，但使用floor的误差在低层minmap更小
		textureWidth = Math.floor(lastTextureWidth / 2);
		textureHeight = Math.floor(lastTextureHeight / 2);
		let framebuffer = new FBO(gl, 1, textureWidth, textureHeight);
		// 这个lastMinmapUVMap用于修正上层minmap的uv和下层uv的关系
		let lastMinmapUVMap = [textureWidth * 2. / lastTextureWidth, textureHeight * 2. / lastTextureHeight];
		framebuffer.lastMinmapUVMap = lastMinmapUVMap;

		lastTextureWidth = textureWidth;
		lastTextureHeight = textureHeight;
		camera.minValueMinmapFBOs.push(framebuffer);
	}
	// console.log("level: " + camera.minValueMinmapFBOs.length);
	// Edit end
