...
void main(void) {
  ...
  float light_atten_coefficient = uLightIntensity / length(uLightPos - vFragPos); 
  vec3 diffuse = diff * light_atten_coefficient * color;

  vec3 viewDir = normalize(uCameraPos - vFragPos);
  float spec = 0.0;
  vec3 reflectDir = reflect(-lightDir, normal);
  spec = pow(max(dot(viewDir, reflectDir), 0.0), 35.0);
  vec3 specular = uKs * light_atten_coefficient  * spec;
